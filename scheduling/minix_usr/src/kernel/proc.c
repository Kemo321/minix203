 ...
 */
  ...
  /* Check if there are any user processes ready */
  if (rdy_head[USER_Q] == NIL_PROC) return;

  /* Get the group of the current process */
  ...
  rdy_tail[USER_Q]->p_nextready = NIL_PROC;
    ...
    /* Look for a process from a different group */
    next_group = (current_group + 1) % 3; /* Start with the next group */
    for (i = 0; i < 3; i++) {
        search_group = (next_group + i) % 3;

        /* Traverse the user queue to find a process from this group */
        rp = rdy_head[USER_Q];
        prev = NULL;
        while (rp != NIL_PROC) {
            if (rp->group == search_group) {
                /* Found a process from the desired group; promote it */
                if (prev != NULL) {
                    /* Detach rp from its current position */
                    prev->p_nextready = rp->p_nextready;
                } else {
                    /* If rp is already at the head, no promotion needed */
                    break;
                }

                /* Move rp to the head of the queue */
                rp->p_nextready = rdy_head[USER_Q];
                rdy_head[USER_Q] = rp;
        ...
        /* If we found and promoted a process, break out of the loop */
        if (rp != NIL_PROC && rp->group == search_group) break;
    }

    /* Call pick_proc to select the next process to run */
    pick_proc();
}
