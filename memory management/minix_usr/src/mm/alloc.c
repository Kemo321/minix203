 ...
 */
  ...
  else
  {
    ...
    if (max_size > 0)
    {
      ...
      biggest_found->h_base += clicks;	/* bite a piece off */
      biggest_found->h_len -= clicks;	/* ditto */
...
PUBLIC int do_hole_map()
{
  ...
  while (hp != NIL_HOLE && hp->h_base < swap_base && i < (mm_in.m1_i1 -1 ))
  {
    ...
    buffer[1] = hp->h_base;
    hp=hp->h_next;
    sys_copy(MM_PROC_NR, D, (phys_bytes) buffer, who, D, (phys_bytes) (mm_in.m1_p1 + (8*i)),
    (phys_bytes) 8);
    i++;
  }
  ...
  sys_copy(MM_PROC_NR, D, (phys_bytes) buffer, who, D,
   (phys_bytes) (mm_in.m1_p1+(8 * i)), (phys_bytes) 4);
